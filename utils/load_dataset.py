...
class DatasetLoader:
    def __init__(self, base_path='GEIL_Data'):
        ...
        Args:
            base_path (str): 存储数据集的根目录路径。
        """
        self.base_path = base_path

    def load_dataset(self, dataset_name):
        """
        根据数据集名称加载并返回脏数据和干净数据表。

        Args:
            dataset_name (str): 要加载的数据集名称，例如 'Beers'。

        Returns:
            tuple: 包含两个pandas DataFrame的元组 (dirty_table, clean_table)。
                   如果数据集不存在或加载失败，则返回 (None, None)。
        """
        dirty_table = None
        clean_table = None
