...
class DetectorRule:
    ...
    def generate_and_validate_rules(
            ...
            if not overwrite_func_list and os.path.exists(func_list_filepath):
                print(f"Loading existing function_list from {func_list_filepath}.")
                function_list = np.load(func_list_filepath, allow_pickle=True).item()
            else:
                print("Creating new function_list.")
                function_list = {}

            for i in np.where(np.array(outlier).sum(axis=0))[0]:
                print(f"\n--- Processing column: {dirty_table.columns[i]} (Index: {i}) ---")
                ...
                if dirty_list:
                    ...
                    for detector_time in range(max_detector_retries):
                        ...
                        try:
                            ...
                            print(f"LLM interaction or function extraction failed: {e}. Retrying...")
                        ...
                        if detector_f1 > self.detector_thres:
                            ...
                            print(f'Function failed! Dataset: {dataset_name}, Attribute: {col_name}, F1: {detector_f1:.4f}, Attempts: {detector_time + 1}.')
            ...
            print(f"function_list saved to: {func_list_filepath}")
